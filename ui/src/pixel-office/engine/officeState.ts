...
export class OfficeState {
  ...
  addAgent(
    ...
    if (seatId) {
      const seat = this.seats.get(seatId)!;
      seat.assigned = true;
      ch = createCharacter(id, palette, seatId, seat, hueShift, role);
    } else {
      ...
      const spawn =
        this.walkableTiles.length > 0
          ...
          : { col: 1, row: 1 };
      ch = createCharacter(id, palette, null, null, hueShift, role);
      ch.x = spawn.col * TILE_SIZE + TILE_SIZE / 2;
      ch.y = spawn.row * TILE_SIZE + TILE_SIZE / 2;
      ch.tileCol = spawn.col;
      ch.tileRow = spawn.row;
  ...
  addSubagent(parentAgentId: number, parentToolId: string): number {
    ...
    if (bestSeatId) {
      ...
      seat.assigned = true;
      ch = createCharacter(id, palette, bestSeatId, seat, hueShift, 'Sub-agent');
    } else {
      ...
      ch = createCharacter(id, palette, null, null, hueShift, 'Sub-agent');
      ch.x = spawn.col * TILE_SIZE + TILE_SIZE / 2;
      ch.y = spawn.row * TILE_SIZE + TILE_SIZE / 2;
      ch.tileCol = spawn.col;
      ch.tileRow = spawn.row;
