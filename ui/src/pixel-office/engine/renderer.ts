...
export function renderRoles(
  ...
  const fontSize = Math.max(8, 5 * zoom);
  ...
  for (const ch of characters) {
    ...
    // Position: centered above the character's head
    // Adjust Y to be above the character but below the speech bubble if both exist
    const labelX = offsetX + ch.x * zoom;
    const labelY = offsetY + (ch.y + sittingOff - 22) * zoom; // 22 is head height roughly

    // Draw text with a subtle outline for visibility on any background
    ctx.strokeStyle = 'rgba(0, 0, 0, 0.8)';
    ctx.lineWidth = Math.max(1, zoom * 0.5);
    ctx.strokeText(ch.role, labelX, labelY);
    
    ctx.fillStyle = '#ffffff';
    ctx.fillText(ch.role, labelX, labelY);
  }
  ctx.restore();
}
...
export function renderFrame(
  ...
  // Character roles
  renderRoles(ctx, characters, offsetX, offsetY, zoom);
